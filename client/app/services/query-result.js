...
class QueryResult {
  ...
  loadLatestCachedResult(queryId, parameters, apiKey) {
    // 构建请求配置，添加API密钥到headers
    const config = {};
    if (apiKey) {
    ...
    axios
      .post(`api/queries/${queryId}/results`, { queryId, parameters }, config)
      .then((response) => {
  ...
  refreshStatus(query, parameters, apiKey, tryNumber = 1) {
    // 构建请求配置，添加API密钥到headers
    const config = {};
    if (apiKey) {
      config.headers = { 'Authorization': `Key ${apiKey}` };
    ...
    const loadResult = () => {
      if (Auth.isAuthenticated()) {
        ...
        this.loadLatestCachedResult(query, parameters, apiKey);
      }
    };

    const request = Auth.isAuthenticated()
      ? axios.get(`api/jobs/${this.job.id}`, config)
      : axios.get(`api/queries/${query}/jobs/${this.job.id}`, config);
    ...
    request
      .then((jobResponse) => {
        ...
        if (this.getStatus() === "processing" && this.job.query_result_id && this.job.query_result_id !== "None") {
          ...
          setTimeout(() => {
            this.refreshStatus(query, parameters, apiKey, tryNumber + 1);
          }, waitTime);
        }
      })
      .catch((error) => {
  ...
  static getByQueryId(id, parameters, applyAutoLimit, maxAge, apiKey) {
    const queryResult = new QueryResult();
    ...
    axios
      ...
      .then((response) => {
        ...
        if ("job" in response) {
          queryResult.refreshStatus(id, parameters, apiKey);
        }
      })
      .catch((error) => {
        handleErrorResponse(queryResult, error);
